...
# ریست کارخانه‌ای (Factory Reset)

⚠️⚠️ **مخرب و غیرقابل بازگشت.** کل دیتای کاربری رو پاک می‌کنه و اپ رو به حالت «بار اول» برمی‌گردونه. ممکنه موقتی باشه — قبل از استفاده در production حتماً یک بک‌آپ بگیر (`POST /admin/backups`).

چه چیزی **پاک** می‌شه:
- همه‌ی کاربران **به‌جز حساب‌های ادمین** (`isAdmin = true`)
- همه‌ی peerهای WireGuard — هم رکورد DB هم کانفیگ روی اینترفیس زنده‌ی `wg0` (کاربر باید کانفیگ جدید بگیره)
- اشتراک‌ها، پرداخت‌ها، رسیدها، نظرات، رویدادهای آماری، refresh tokenها (شامل توکن‌های ادمین → ادمین باید دوباره login کنه)
- `AppSettings` به مقدار پیش‌فرض ریست می‌شه (مثلاً `adsEnabled = true`)

چه چیزی **می‌ماند**:
- کاتالوگ اپ‌های ایرانی (داده‌ی seed)
- حساب‌های ادمین (فقط رکورد کاربر؛ اشتراک/peer/توکنشون هم پاک می‌شه)

## POST `/api/admin/factory-reset`
Body — `FactoryResetRequest`:
```json
{ "password": "<رمز اختصاصی ریست>" }
```
> رمز جدا از رمز login ادمینه و به‌صورت hash (PBKDF2-SHA256) در سرور نگهداری می‌شه — plaintext هیچ‌جا ذخیره نمی‌شه.

Response 200 — `FactoryResetResult`:
```json
{
  "usersDeleted": 152,
  "peersRemoved": 148,
  "peersRemovedFromInterface": 147
}
```
- `usersDeleted` — تعداد کاربران غیرادمین حذف‌شده
- `peersRemoved` — رکوردهای peer حذف‌شده از DB
- `peersRemovedFromInterface` — peerهایی که از `wg0` زنده هم حذف شدن (اگر کمتر از `peersRemoved` بود یعنی حذف چند peer از اینترفیس خطا داده ولی رکورد DB پاک شده)

خطا: 401 `{ "error": "رمز نامعتبر است" }` · 500 در صورت خطای داخلی.

> **UX پیشنهادی:** دکمه‌ی قرمز جدا در بخش خطرناک تنظیمات + مودال تأیید که رمز اختصاصی رو می‌گیره + تیک «می‌دانم این عمل برگشت‌ناپذیر است». بعد از موفقیت، ادمین رو logout و به صفحه‌ی login بفرست (توکن‌ها باطل شدن).
